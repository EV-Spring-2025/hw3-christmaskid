...
ti.init(arch=ti.cuda, device_memory_GB=7.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--material", type=str, default=None, 
        choices=[None, "jelly", "metal", "sand", "foam", "snow", "plasticine"])
    parser.add_argument("--n_grid", type=int, default=None)
    parser.add_argument("--substep_dt", type=float, default=None)
    parser.add_argument("--grid_v_damping_scale", type=float, default=None)
    ...
    (
        ...
        camera_params,
    ) = decode_param_json(args.config, args.material, args.n_grid, 
                          args.substep_dt, args.grid_v_damping_scale, args.softening)

    print(f"Using material: {material_params['material']}")
    ...
    for frame in tqdm(range(frame_num)):
        ...
        if args.render_img:
            ...
            del colors_precomp, rendering, cv2_img, pos, cov3D, rot, opacity, shs
            gc.collect()
